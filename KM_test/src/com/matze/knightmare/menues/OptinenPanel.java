...
public class OptinenPanel extends JPanel implements ChangeListener, ActionListener {
	...
	public OptinenPanel(int width, int height) {
...
//		volume.setBounds(width / 4, (optionen.length - 1) * height / (optionen.length + 1), width / 2, height / (optionen.length + 1));
...
//			optionen[i].setBounds(width / 4, (i < optionen.length - 1 ? i : i + 1) * height / (optionen.length + 1), width / 2, height / (optionen.length + 1));
		...
		zurück.setBounds(width - width / 8, height - width / 24, width / 8, width / 24);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (q == optionen[2]) {
			MainFrame.instance.changeTo(5);
		}

		if (q == optionen[1]) {
			MainFrame.instance.changeTo(6);
		}

		if (q == optionen[0]) {
			MainFrame.instance.changeTo(7);
		}
